...
public class RefreshHelper<T> {
    ...
    //执行默认刷新 mPageIndex变为 一
    public void onDefaultMRefresh(boolean isShowDialog) {
        mPageIndex = 1;
        if (mRefreshInterface != null) {
    ...
    //执行默认刷新 mPageIndex++
    public void onDefaultMLoadMore(boolean isShowDialog) {
        if (mDataList.size() > 0) {
            mPageIndex++;
        }
        if (mRefreshInterface != null) {
    ...
    public void setEmptyView(String emp, int img){
        if (mEmptyView != null && mDataList.isEmpty()) {
            if (mRefreshInterface != null) {
                mRefreshInterface.showEmptyState(emp, img);
            }
